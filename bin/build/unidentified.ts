...
const generateUnidentifiedWordLists = async (id: string, flatLexicon: FlatLexicon): Promise<void> => {
  ...
  if (dataRead && analysisRead) {
    ...
    const path = analysisPath.replace('build/analysis', 'tmp/lemmas').replace('.json', '.txt')
...
export const unidentifiedWords = (analysis: Analysis, flatLexicon: FlatLexicon): string => analysis.lemmas
  .map(x => x.label)
