...
const texts = authors.reduce((acc: TextStub[], current) => [...acc, ...current.texts], [])
...
const maleTexts = men.reduce((acc: TextStub[], current) => [...acc, ...current.texts], [])
...
const femaleTexts = women.reduce((acc: TextStub[], current) => [...acc, ...current.texts], [])
...
const totalImportedTexts = importedTexts.length - (duplicatedImportedTexts.length / 2)
console.log(`Imported texts: ${totalImportedTexts} (${percent(totalImportedTexts, totalTexts)})`)
