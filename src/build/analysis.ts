...
export const analyse = (
  ...
  if (imported && isText(data)) {
    for (const block of data.blocks.filter((x) => x.type !== "title")) {
      ...
      const nameWordCount = result.names.reduce(
        (x: number, y: string) => x + y.split(" ").length,
        0
      );
      analysis.nameWordCount += nameWordCount;
      analysis.wordCount += nameWordCount;

      const foreignWordCount = result.foreignText.reduce(
        (x: number, y: string) => x + y.split(" ").length,
        0
      );
      ...
      const citationWordCount = result.citations.reduce(
        (x: number, y: string) => x + y.split(" ").length,
        0
      );
      ...
      const marginCommentWordCount = result.marginComments.reduce(
        (x: number, y: string) => x + y.split(" ").length,
        0
      );
  ...
  for (const subAnalysis of subAnalyses) {
    if (
      data.id === "index" ||
      subAnalysis.id.split(".")[0] === data.id.split(".")[0]
    ) {
      // don't count subtexts by different authors
...
export const lemmatize = (
  ...
  const foreignText = foreignTextCheck
    ? foreignTextCheck.map((x) => x.slice(1, -1))
    : [];

  const citationsCheck = content.match(/\[(.*?)\]/g);
  const citations = citationsCheck
    ? citationsCheck.map((x) => x.slice(1, -1))
    : [];

  const marginCommentsCheck = content.match(/#(.*?)#/g);
  const marginComments = marginCommentsCheck
    ? marginCommentsCheck.map((x) => x.slice(1, -1))
    : [];
  ...
  const lemmas = markit
    .content(strippedContent.replace(/([^~\\])~([^~])/g, "$1&126;$2"), {
      format: "txt",
    })
    ...
    .filter((x) => {
      // get rid of numbers, and put them in the numbers array
      if (isNaN(parseInt(x))) {
        return true;
      } else {
        numbers.push(x);
