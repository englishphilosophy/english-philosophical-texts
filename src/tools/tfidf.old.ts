...
export const addTfIdf = (
  analysis: Analysis,
  indexAnalysis: Analysis
): Analysis => {
  for (const lemma of analysis.lemmas) {
    const indexLemma = indexAnalysis.lemmas.find(
      (x) => x.label === lemma.label
    );
    const df = indexLemma ? indexLemma.documentFrequency : 0;
    const rawIdf = df - lemma.documentFrequency + 1;
    const totalDocuments =
      indexAnalysis.importedDocumentCount - analysis.importedDocumentCount;
